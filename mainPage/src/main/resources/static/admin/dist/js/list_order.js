$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            { label: "배송 주소", name: "userAddress", index: "userAddress", width: 10, hidden: true },
    ...
    function operateFormatter(cellvalue, rowObject) {
        return (
            "<a href='##' onclick=openOrderItems(" +
            rowObject.rowId +
            ")>주문 상세정보</a>" +
            "<br>" +
            "<a href='##' onclick=openExpressInfo(" +
            rowObject.rowId +
            ")>구매자 상세정보</a>"
        );
    }
...
function orderCheckDone() {
    ...
    if ((orderNos.length > 0) & (orderNos.length < 100)) {
        swal(orderNos + "결제완료 상태가 아닙니다 성공적으로 포장완료 작업을 수행할 수 없습니다.", {
            icon: "error",
        });
        return;
    }
    if (orderNos.length >= 100) {
        swal("너무 많은 결제완료 상태가 아닌 주문들을 선택했습니다. 포장완료 작업을 수행할 수 없습니다.", {
            icon: "error",
        });
        return;
    }
...
function orderCheckOut() {
    ...
    if ((orderNos.length > 0) & (orderNos.length < 100)) {
        swal(orderNos + "포장완료 상태가 아닙니다 성공적으로 배송완료 작업을 수행할 수 없습니다.", {
            icon: "error",
        });
        return;
    }
    if (orderNos.length >= 100) {
        swal("너무 많은 포장완료 상태가 아닌 주문들을 선택했습니다. 배송완료 작업을 수행할 수 없습니다.", {
            icon: "error",
        });
        return;
    }
