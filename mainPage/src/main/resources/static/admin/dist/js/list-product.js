$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            { label: "재고수량", name: "stockNum", index: "stockNum", width: 60 },
    ...
    function productSellStatusFormatter(cellvalue) {
        // 상품 진열 상태(0: 진열안함 1: 진열)
        if (cellvalue == 0) {
            return '<button type="button" class="btn btn-block btn-success btn-sm" style="width: 80%;">판매중</button>';
        }
        if (cellvalue == 1) {
            return '<button type="button" class="btn btn-block btn-secondary btn-sm" style="width: 80%;">진열안함</button>';
        }
    }
